...
export default function HRApprovalPage() {
  ...
  useEffect(() => {
    async function load() {
      const data = await getPendingApprovalsForRole("hr");
      setRequests(data);
      setLoading(false);
    }
    load();
  ...
  const stats = useMemo(() => {
    ...
    return { escalated, total: requests.length };
  }, [requests]);
  ...
  const handleApprove = async (req) => {
    ...
    try {
      const updated = await approveLeaveRequest(req.id, "hr", "Approved by HR");
  ...
  const handleRemarksConfirm = async (remarks) => {
    ...
    try {
      if (action === "reject") {
        const updated = await rejectLeaveRequest(req.id, "hr", remarks);
        toast.success(`Leave ${updated.id} rejected`);
      } else {
        const updated = await requestClarificationOnLeave(req.id, "hr", remarks);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="HR Approvals"
        description="Review escalated leave requests that require HR approval."
      />
      ...
      {filtered.length === 0 ? (
        <Card>
          <EmptyState
            ...
            title="All clear!"
            description="No escalated leave requests pending your approval."
          />
        </Card>
      ) : (
        <>
